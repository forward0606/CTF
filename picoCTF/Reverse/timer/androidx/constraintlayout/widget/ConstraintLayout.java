...
public class ConstraintLayout extends ViewGroup {
        ...
        */
        public final void measure(ConstraintWidget widget, BasicMeasure.Measure measure) {
            ...
            if (widget.getVisibility() == 8 && !widget.isInPlaceholder()) {
                ...
                switch (AnonymousClass1.$SwitchMap$androidx$constraintlayout$core$widgets$ConstraintWidget$DimensionBehaviour[horizontalBehavior.ordinal()]) {
                    case 1:
                    ...
                    case 4:
                        ...
                        if (measure.measureStrategy == BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS || measure.measureStrategy == BasicMeasure.Measure.USE_GIVEN_DIMENSIONS) {
                            boolean otherDimensionStable = child.getMeasuredHeight() == widget.getHeight() ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                ...
                switch (AnonymousClass1.$SwitchMap$androidx$constraintlayout$core$widgets$ConstraintWidget$DimensionBehaviour[verticalBehavior.ordinal()]) {
                    case 1:
                ...
                if (container != null && Optimizer.enabled(ConstraintLayout.this.mOptimizationLevel, 256) && child.getMeasuredWidth() == widget.getWidth() && child.getMeasuredWidth() < container.getWidth() && child.getMeasuredHeight() == widget.getHeight() && child.getMeasuredHeight() < container.getHeight() && child.getBaseline() == widget.getBaselineDistance() && !widget.isMeasureRequested()) {
                    boolean similar = (isSimilarSpec(widget.getLastHorizontalMeasureSpec(), horizontalSpec2, widget.getWidth()) && isSimilarSpec(widget.getLastVerticalMeasureSpec(), verticalSpec4, widget.getHeight())) ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                    if (similar) {
                    ...
                    if (verticalMatchConstraints && widget.mDimensionRatio > 0.0f) {
                        ...
                        if (verticalDimension2 != BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS && measure.measureStrategy != BasicMeasure.Measure.USE_GIVEN_DIMENSIONS && horizontalMatchConstraints && widget.mMatchConstraintDefaultWidth == 0 && verticalMatchConstraints && widget.mMatchConstraintDefaultHeight == 0) {
                            ...
                            if ((child instanceof VirtualLayout) && (widget instanceof androidx.constraintlayout.core.widgets.VirtualLayout)) {
                                ...
                                ((VirtualLayout) child).onMeasure(layout, horizontalSpec2, verticalSpec4);
                            } else {
                                child.measure(horizontalSpec2, verticalSpec4);
                            }
                            widget.setLastMeasureSpec(horizontalSpec2, verticalSpec4);
                            ...
                            if (horizontalSpec3 > 0) {
                                verticalSpec = verticalSpec4;
                                int width2 = Math.max(widget.mMatchConstraintMinWidth, w);
                                width = width2;
                            } else {
                                verticalSpec = verticalSpec4;
                                width = w;
                            ...
                            if (w == width && h == widthPadding) {
                                ...
                                if (w == width) {
                                    ...
                                    i = BasicMeasure.EXACTLY;
                                    horizontalSpec = View.MeasureSpec.makeMeasureSpec(width, BasicMeasure.EXACTLY);
                                }
                                if (h == widthPadding) {
                                    verticalSpec2 = verticalSpec;
                                } else {
                                    verticalSpec2 = View.MeasureSpec.makeMeasureSpec(widthPadding, i);
                                }
                                child.measure(horizontalSpec, verticalSpec2);
                                widget.setLastMeasureSpec(horizontalSpec, verticalSpec2);
                                int width4 = child.getMeasuredWidth();
                                int height3 = child.getMeasuredHeight();
                                int baseline3 = child.getBaseline();
                                verticalSpec3 = width4;
                                widthPadding = height3;
                                baseline = baseline3;
                            }
                        }
                        boolean hasBaseline = baseline != -1 ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                        measure.measuredNeedsSolverPass = (verticalSpec3 == measure.horizontalDimension && widthPadding == measure.verticalDimension) ? false : ConstraintLayout.USE_CONSTRAINTS_HELPER;
                        ...
                        measure.measuredWidth = verticalSpec3;
    ...
    */
    public void applyConstraintsFromLayoutParams(boolean isInEditMode, View child, ConstraintWidget widget, LayoutParams layoutParams, SparseArray<ConstraintWidget> idToWidget) {
        ...
        if (Build.VERSION.SDK_INT >= 17) {
            ...
            if (resolvedLeftToLeft2 == -1 && resolvedLeftToRight4 == -1) {
                if (layoutParams.startToStart != -1) {
                    ...
                    int resolvedLeftToRight5 = layoutParams.startToEnd;
                    resolvedLeftToRight2 = resolvedLeftToRight5;
                }
        ...
        int resolvedLeftToLeft3 = layoutParams.circleConstraint;
        if (resolvedLeftToLeft3 != -1) {
            ...
            if (resolveGoneRightMargin2 != -1) {
                ConstraintWidget target6 = idToWidget.get(resolveGoneRightMargin2);
                if (target6 == null) {
            ...
            if (resolvedRightToLeft != -1) {
                ...
                int resolvedRightToRight6 = resolvedRightToRight;
                if (resolvedRightToRight6 != -1 && (target2 = idToWidget.get(resolvedRightToRight6)) != null) {
                    int resolvedRightToRight7 = resolveGoneRightMargin;
                    widget.immediateConnect(ConstraintAnchor.Type.RIGHT, target2, ConstraintAnchor.Type.RIGHT, layoutParams.rightMargin, resolvedRightToRight7);
                }
            }
